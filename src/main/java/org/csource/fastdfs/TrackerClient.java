...
public class TrackerClient {
	 ...
	 * @throws Exception 
	 */
	public StorageServer getStoreStorage(TrackerServer trackerServer) throws Exception {
	 ...
	 * @throws Exception 
	 */
	public StorageServer getStoreStorage(TrackerServer trackerServer, String groupName) throws Exception {
		byte[] header;
		String ip_addr;
		int port;
		byte cmd;
		...
		try {
			...
			if (!bNewConnection) {
				try {
					...
					logger.error("Exception when trying to close trackerServer", ex1);
				}
			}
	 ...
	 * @throws Exception 
	 */
	public StorageServer[] getStoreStorages(TrackerServer trackerServer, String groupName) throws Exception {
		byte[] header;
		String ip_addr;
		int port;
		byte cmd;
	...
	public ServerInfo[] getFetchStorages(TrackerServer trackerServer, String groupName, String filename)
			throws Exception {
		return this.getStorages(trackerServer, ProtoCommon.TRACKER_PROTO_CMD_SERVICE_QUERY_FETCH_ALL, groupName,
				filename);
	}
	 ...
	 * @throws Exception 
	 */
	public StorageServer getFetchStorage1(TrackerServer trackerServer, String file_id) throws Exception {
	 ...
	 * @throws Exception 
	 */
	public ServerInfo[] getFetchStorages1(TrackerServer trackerServer, String file_id) throws Exception {
		String[] parts = new String[2];
		this.errno = StorageClient1.split_file_id(file_id, parts);
		if (this.errno != 0) {
			return null;
	 ...
	 * @throws Exception 
	 */
	public StructGroupStat[] listGroups(TrackerServer trackerServer) throws Exception {
		byte[] header;
		//String ip_addr;
		//int port;
		//byte cmd;
		...
		try {
			...
			logger.error("Exception when trying to list Groups", ex);
			this.errno = ProtoCommon.ERR_NO_EINVAL;
			return null;
		} finally {
			if (bNewConnection) {
	 ...
	 * @throws Exception 
	 */
	public StructStorageStat[] listStorages(TrackerServer trackerServer, String groupName) throws Exception {
	...
	public StructStorageStat[] listStorages(TrackerServer trackerServer, String groupName, String storageIpAddr)
		...
		try {
			...
			logger.error("Exception when trying to list storages", ex);
			this.errno = ProtoCommon.ERR_NO_EINVAL;
			return null;
		} finally {
			if (bNewConnection) {
	 ...
	 * @throws Exception 
	 */
	public boolean deleteStorage(TrackerGroup trackerGroup, String groupName, String storageIpAddr) throws Exception {
		int serverIndex;
		int notFoundCount;
		...
		for (serverIndex = 0; serverIndex < trackerGroup.tracker_servers.length; serverIndex++) {
			try {
				...
				logger.error("Exception occured while trying to get connection from tracker group", ex);
